...
def note_agent_node(state: State, agent: AgentExecutor, name: str) -> State:
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"JSON parse failed: {e}. Attempting to create valid structure from output.")
            # If JSON parsing fails, create a simple structure with the output as content
            parsed_output = {
                "messages": [{"type": "ai", "content": cleaned_output}],
                "hypothesis": "",
                "process": "",
                "process_decision": "",
                "visualization_state": "",
...
def refiner_node(state: State, agent: AgentExecutor, name: str) -> State:
    ...
    try:
        ...
        state["messages"].append(AIMessage(content=result_content, name=name))
        state["sender"] = name
